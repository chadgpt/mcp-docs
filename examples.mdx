...
此页面展示了各种模型上下文协议（MCP）服务器，展示了协议的功能和多样性。这些服务器使大型语言模型（LLM）能够安全地访问工具和数据源。

## 参考实现

这些官方参考服务器展示了核心 MCP 功能和 SDK 的使用：
...
- **[Fetch](https://github.com/modelcontextprotocol/servers/tree/main/src/fetch)** - 为 LLM 使用优化的 Web 内容获取和转换
- **[Puppeteer](https://github.com/modelcontextprotocol/servers/tree/main/src/puppeteer)** - 浏览器自动化和 Web 抓取功能
...
- **[Search1API](https://github.com/fatwang2/search1api-mcp)** - 搜索、抓取和站点地图的统一 API
...
> **注意:** 社区服务器未经测试，使用时需自担风险。它们与 Anthropic 无关，也未得到其认可。

有关社区服务器的完整列表，请访问 [MCP 服务器库](https://github.com/modelcontextprotocol/servers)。
...
- [MCP 服务器库](https://github.com/modelcontextprotocol/servers) - 参考实现和社区服务器的完整集合
